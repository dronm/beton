...
//throw new Exception('TEST');
		...
		foreach($events as $ev){
			...
			}else if(mb_strpos($material_name, 'щебен') !== FALSE || mb_strpos($material_name, '5-20') !== FALSE){
			...
			if(isset($materals[$material_name])){
				...
				$material_ar = $dbLink->query_first(sprintf("SELECT id FROM raw_materials WHERE name='%s' LIMIT 1", $material_name));
