<?php
/**
	DO NOT MODIFY THIS FILE!	
	Its content is generated automaticaly from template placed at build/permissions/permission_php.tmpl.	
 */
function method_allowed($contrId,$methId){
$permissions = array();

					$permissions['Event_Controller_subscribe']=TRUE;
				
					$permissions['Event_Controller_unsubscribe']=TRUE;
				
					$permissions['Event_Controller_publish']=TRUE;
				
					$permissions['Client_Controller_insert']=TRUE;
				
					$permissions['Client_Controller_update']=TRUE;
				
					$permissions['Client_Controller_delete']=TRUE;
				
					$permissions['Client_Controller_get_list']=TRUE;
				
					$permissions['Client_Controller_get_object']=TRUE;
				
					$permissions['Client_Controller_complete']=TRUE;
				
					$permissions['Client_Controller_complete_for_order']=TRUE;
				
					$permissions['Client_Controller_union']=TRUE;
				
					$permissions['Client_Controller_set_duplicate_valid']=TRUE;
				
					$permissions['Client_Controller_get_duplicates_list']=TRUE;
				
					$permissions['Client_Controller_insert_from_order']=TRUE;
				
					$permissions['ConcreteType_Controller_insert']=TRUE;
				
					$permissions['ConcreteType_Controller_update']=TRUE;
				
					$permissions['ConcreteType_Controller_delete']=TRUE;
				
					$permissions['ConcreteType_Controller_get_list']=TRUE;
				
					$permissions['ConcreteType_Controller_get_object']=TRUE;
				
					$permissions['ConcreteType_Controller_complete']=TRUE;
				
					$permissions['ConcreteType_Controller_get_list_for_lab']=TRUE;
				
					$permissions['ConcreteType_Controller_get_for_client_list']=TRUE;
				
					$permissions['ConcreteType_Controller_get_for_site_list']=TRUE;
				
					$permissions['Destination_Controller_insert']=TRUE;
				
					$permissions['Destination_Controller_update']=TRUE;
				
					$permissions['Destination_Controller_delete']=TRUE;
				
					$permissions['Destination_Controller_get_list']=TRUE;
				
					$permissions['Destination_Controller_get_object']=TRUE;
				
					$permissions['Destination_Controller_complete_dest']=TRUE;
				
					$permissions['Destination_Controller_complete_for_order']=TRUE;
				
					$permissions['Destination_Controller_complete_for_site']=TRUE;
				
					$permissions['Destination_Controller_get_for_client_list']=TRUE;
				
					$permissions['Destination_Controller_get_coords_on_name']=TRUE;
				
					$permissions['Destination_Controller_get_route_to_zone']=TRUE;
				
					$permissions['Destination_Controller_at_dest_avg_time']=TRUE;
				
					$permissions['Destination_Controller_route_to_dest_avg_time']=TRUE;
				
					$permissions['Vehicle_Controller_insert']=TRUE;
				
					$permissions['Vehicle_Controller_update']=TRUE;
				
					$permissions['Vehicle_Controller_delete']=TRUE;
				
					$permissions['Vehicle_Controller_get_list']=TRUE;
				
					$permissions['Vehicle_Controller_get_object']=TRUE;
				
					$permissions['Vehicle_Controller_complete']=TRUE;
				
					$permissions['Vehicle_Controller_get_vehicle_statistics']=TRUE;
				
					$permissions['Vehicle_Controller_complete_features']=TRUE;
				
					$permissions['Vehicle_Controller_complete_makes']=TRUE;
				
					$permissions['Vehicle_Controller_check_for_broken_trackers']=TRUE;
				
					$permissions['Vehicle_Controller_vehicles_with_trackers']=TRUE;
				
					$permissions['Vehicle_Controller_get_current_position']=TRUE;
				
					$permissions['Vehicle_Controller_get_current_position_all']=TRUE;
				
					$permissions['Vehicle_Controller_get_route']=TRUE;
				
					$permissions['Vehicle_Controller_get_track']=TRUE;
				
					$permissions['Vehicle_Controller_get_tool_tip']=TRUE;
				
					$permissions['Vehicle_Controller_get_stops_at_dest']=TRUE;
				
					$permissions['Vehicle_Controller_get_total_shipped']=TRUE;
				
					$permissions['OrderGarbage_Controller_delete']=TRUE;
				
					$permissions['OrderGarbage_Controller_get_list']=TRUE;
				
					$permissions['OrderGarbage_Controller_get_object']=TRUE;
				
					$permissions['Order_Controller_insert']=TRUE;
				
					$permissions['Order_Controller_update']=TRUE;
				
					$permissions['Order_Controller_delete']=TRUE;
				
					$permissions['Order_Controller_get_list']=TRUE;
				
					$permissions['Order_Controller_get_object']=TRUE;
				
					$permissions['Order_Controller_get_object_for_lab']=TRUE;
				
					$permissions['Order_Controller_get_make_orders_form']=TRUE;
				
					$permissions['Order_Controller_get_make_orders_form_ord']=TRUE;
				
					$permissions['Order_Controller_get_make_orders_form_veh']=TRUE;
				
					$permissions['Order_Controller_get_make_orders_form_mat']=TRUE;
				
					$permissions['Order_Controller_get_make_orders_for_weighing_form']=TRUE;
				
					$permissions['Order_Controller_get_make_orders_for_lab_form']=TRUE;
				
					$permissions['Order_Controller_get_make_orders_for_lab_form_ord']=TRUE;
				
					$permissions['Order_Controller_get_make_orders_for_lab_form_veh']=TRUE;
				
					$permissions['Order_Controller_get_make_orders_for_lab_form_mat']=TRUE;
				
					$permissions['Order_Controller_get_make_orders_list']=TRUE;
				
					$permissions['Order_Controller_get_make_orders_for_lab_list']=TRUE;
				
					$permissions['Order_Controller_get_avail_spots']=TRUE;
				
					$permissions['Order_Controller_complete_descr']=TRUE;
				
					$permissions['Order_Controller_get_comment']=TRUE;
				
					$permissions['Order_Controller_fields_from_client_order']=TRUE;
				
					$permissions['Order_Controller_set_payed']=TRUE;
				
					$permissions['Order_Controller_complete']=TRUE;
				
					$permissions['Order_Controller_get_list_for_client']=TRUE;
				
					$permissions['Order_Controller_calc_for_site']=TRUE;
				
					$permissions['Driver_Controller_insert']=TRUE;
				
					$permissions['Driver_Controller_update']=TRUE;
				
					$permissions['Driver_Controller_delete']=TRUE;
				
					$permissions['Driver_Controller_get_list']=TRUE;
				
					$permissions['Driver_Controller_get_object']=TRUE;
				
					$permissions['Driver_Controller_complete']=TRUE;
				
					$permissions['Driver_Controller_driver_cheat_report']=TRUE;
				
					$permissions['RawMaterial_Controller_insert']=TRUE;
				
					$permissions['RawMaterial_Controller_update']=TRUE;
				
					$permissions['RawMaterial_Controller_delete']=TRUE;
				
					$permissions['RawMaterial_Controller_get_list']=TRUE;
				
					$permissions['RawMaterial_Controller_get_object']=TRUE;
				
					$permissions['RawMaterial_Controller_complete']=TRUE;
				
					$permissions['RawMaterial_Controller_get_list_for_concrete']=TRUE;
				
					$permissions['RawMaterial_Controller_supplier_orders_list']=TRUE;
				
					$permissions['RawMaterial_Controller_update_procurement']=TRUE;
				
					$permissions['RawMaterial_Controller_load_procurement']=TRUE;
				
					$permissions['RawMaterial_Controller_oper_week_report']=TRUE;
				
					$permissions['RawMaterial_Controller_total_report']=TRUE;
				
					$permissions['RawMaterial_Controller_total_details_report']=TRUE;
				
					$permissions['RawMaterial_Controller_set_min_quant']=TRUE;
				
					$permissions['RawMaterial_Controller_correct_consumption']=TRUE;
				
					$permissions['RawMaterial_Controller_correct_obnul']=TRUE;
				
					$permissions['RawMaterial_Controller_correct_list']=TRUE;
				
					$permissions['RawMaterial_Controller_total_list']=TRUE;
				
					$permissions['RawMaterial_Controller_mat_totals']=TRUE;
				
					$permissions['RawMaterial_Controller_get_material_actions_list']=TRUE;
				
					$permissions['RawMaterial_Controller_get_material_actions_by_shift_list']=TRUE;
				
					$permissions['RawMaterial_Controller_get_material_avg_cons_on_ctp']=TRUE;
				
					$permissions['RawMaterial_Controller_get_material_cons_tolerance_violation_list']=TRUE;
				
					$permissions['RawMaterialPrice_Controller_insert']=TRUE;
				
					$permissions['RawMaterialPrice_Controller_update']=TRUE;
				
					$permissions['RawMaterialPrice_Controller_delete']=TRUE;
				
					$permissions['RawMaterialPrice_Controller_get_list']=TRUE;
				
					$permissions['RawMaterialPrice_Controller_get_object']=TRUE;
				
					$permissions['RawMaterialPriceForNorm_Controller_insert']=TRUE;
				
					$permissions['RawMaterialPriceForNorm_Controller_update']=TRUE;
				
					$permissions['RawMaterialPriceForNorm_Controller_delete']=TRUE;
				
					$permissions['RawMaterialPriceForNorm_Controller_get_list']=TRUE;
				
					$permissions['RawMaterialPriceForNorm_Controller_get_object']=TRUE;
				
					$permissions['RawMaterialConsRateDate_Controller_insert']=TRUE;
				
					$permissions['RawMaterialConsRateDate_Controller_update']=TRUE;
				
					$permissions['RawMaterialConsRateDate_Controller_delete']=TRUE;
				
					$permissions['RawMaterialConsRateDate_Controller_get_list']=TRUE;
				
					$permissions['RawMaterialConsRateDate_Controller_get_object']=TRUE;
				
					$permissions['RawMaterialConsRateDate_Controller_recalc_consumption']=TRUE;
				
					$permissions['RawMaterialConsRateDate_Controller_complete']=TRUE;
				
					$permissions['RawMaterialProcurRate_Controller_insert']=TRUE;
				
					$permissions['RawMaterialProcurRate_Controller_update']=TRUE;
				
					$permissions['RawMaterialProcurRate_Controller_delete']=TRUE;
				
					$permissions['RawMaterialProcurRate_Controller_get_list']=TRUE;
				
					$permissions['RawMaterialProcurRate_Controller_get_object']=TRUE;
				
					$permissions['RAMaterialConsumption_Controller_get_dates_list']=TRUE;
				
					$permissions['RAMaterialConsumption_Controller_get_docs_list']=TRUE;
				
					$permissions['RawMaterialConsRate_Controller_update']=TRUE;
				
					$permissions['RawMaterialConsRate_Controller_delete']=TRUE;
				
					$permissions['RawMaterialConsRate_Controller_get_list']=TRUE;
				
					$permissions['RawMaterialConsRate_Controller_get_object']=TRUE;
				
					$permissions['RawMaterialConsRate_Controller_raw_material_cons_report']=TRUE;
				
					$permissions['VehicleSchedule_Controller_insert']=TRUE;
				
					$permissions['VehicleSchedule_Controller_update']=TRUE;
				
					$permissions['VehicleSchedule_Controller_delete']=TRUE;
				
					$permissions['VehicleSchedule_Controller_get_list']=TRUE;
				
					$permissions['VehicleSchedule_Controller_complete']=TRUE;
				
					$permissions['VehicleSchedule_Controller_get_current_veh_list']=TRUE;
				
					$permissions['VehicleSchedule_Controller_get_object']=TRUE;
				
					$permissions['VehicleSchedule_Controller_set_free']=TRUE;
				
					$permissions['VehicleSchedule_Controller_set_production_base']=TRUE;
				
					$permissions['VehicleSchedule_Controller_set_out']=TRUE;
				
					$permissions['VehicleSchedule_Controller_gen_schedule']=TRUE;
				
					$permissions['VehicleSchedule_Controller_get_vehicle_efficiency']=TRUE;
				
					$permissions['VehicleSchedule_Controller_get_schedule_report']=TRUE;
				
					$permissions['VehicleSchedule_Controller_get_schedule_report_all']=TRUE;
				
					$permissions['VehicleSchedule_Controller_get_vehicle_runs']=TRUE;
				
					$permissions['VehicleSchedule_Controller_get_vehicle_owner_schedule']=TRUE;
				
					$permissions['VehicleSchedule_Controller_set_schedule']=TRUE;
				
					$permissions['Shipment_Controller_insert']=TRUE;
				
					$permissions['Shipment_Controller_update']=TRUE;
				
					$permissions['Shipment_Controller_delete']=TRUE;
				
					$permissions['Shipment_Controller_get_list']=TRUE;
				
					$permissions['Shipment_Controller_get_list_for_veh_owner']=TRUE;
				
					$permissions['Shipment_Controller_get_list_for_client']=TRUE;
				
					$permissions['Shipment_Controller_get_list_for_client_veh_owner']=TRUE;
				
					$permissions['Shipment_Controller_get_list_for_order']=TRUE;
				
					$permissions['Shipment_Controller_get_pump_list']=TRUE;
				
					$permissions['Shipment_Controller_get_pump_list_for_veh_owner']=TRUE;
				
					$permissions['Shipment_Controller_get_shipment_date_list']=TRUE;
				
					$permissions['Shipment_Controller_get_object']=TRUE;
				
					$permissions['Shipment_Controller_get_assigned_vehicle_list']=TRUE;
				
					$permissions['Shipment_Controller_get_operator_list']=TRUE;
				
					$permissions['Shipment_Controller_set_shipped']=TRUE;
				
					$permissions['Shipment_Controller_unset_shipped']=TRUE;
				
					$permissions['Shipment_Controller_shipment_report']=TRUE;
				
					$permissions['Shipment_Controller_shipment_invoice']=TRUE;
				
					$permissions['Shipment_Controller_shipment_ttn']=TRUE;
				
					$permissions['Shipment_Controller_shipment_putevoi_list']=TRUE;
				
					$permissions['Shipment_Controller_get_time_list']=TRUE;
				
					$permissions['Shipment_Controller_set_blanks_exist']=TRUE;
				
					$permissions['Shipment_Controller_delete_shipped']=TRUE;
				
					$permissions['Shipment_Controller_delete_assigned']=TRUE;
				
					$permissions['Shipment_Controller_owner_set_agreed']=TRUE;
				
					$permissions['Shipment_Controller_owner_set_agreed_all']=TRUE;
				
					$permissions['Shipment_Controller_owner_set_pump_agreed']=TRUE;
				
					$permissions['Shipment_Controller_owner_set_pump_agreed_all']=TRUE;
				
					$permissions['Shipment_Controller_get_shipped_vihicles_list']=TRUE;
				
					$permissions['Shipment_Controller_complete']=TRUE;
				
					$permissions['Shipment_Controller_get_passport_ship']=TRUE;
				
					$permissions['Shipment_Controller_get_passport_all']=TRUE;
				
					$permissions['Shipment_Controller_get_passport_stamp_ship']=TRUE;
				
					$permissions['Shipment_Controller_get_passport_stamp_all']=TRUE;
				
					$permissions['Login_Controller_get_list']=TRUE;
				
					$permissions['Login_Controller_get_object']=TRUE;
				
					$permissions['LoginDevice_Controller_get_list']=TRUE;
				
					$permissions['LoginDevice_Controller_switch_banned']=TRUE;
				
					$permissions['LoginDeviceBan_Controller_insert']=TRUE;
				
					$permissions['LoginDeviceBan_Controller_delete']=TRUE;
				
					$permissions['LoginDeviceBan_Controller_get_list']=TRUE;
				
					$permissions['LoginDeviceBan_Controller_get_object']=TRUE;
				
					$permissions['User_Controller_insert']=TRUE;
				
					$permissions['User_Controller_update']=TRUE;
				
					$permissions['User_Controller_delete']=TRUE;
				
					$permissions['User_Controller_get_list']=TRUE;
				
					$permissions['User_Controller_get_object']=TRUE;
				
					$permissions['User_Controller_set_new_pwd']=TRUE;
				
					$permissions['User_Controller_reset_pwd']=TRUE;
				
					$permissions['User_Controller_login']=TRUE;
				
					$permissions['User_Controller_login_ext']=TRUE;
				
					$permissions['User_Controller_login_refresh']=TRUE;
				
					$permissions['User_Controller_login_k']=TRUE;
				
					$permissions['User_Controller_login_ks']=TRUE;
				
					$permissions['User_Controller_logout']=TRUE;
				
					$permissions['User_Controller_login_tm']=TRUE;
				
					$permissions['User_Controller_tm_check_tel']=TRUE;
				
					$permissions['User_Controller_tm_send_code']=TRUE;
				
					$permissions['User_Controller_tm_get_left_time']=TRUE;
				
					$permissions['User_Controller_tm_check_code']=TRUE;
				
					$permissions['User_Controller_logout_html']=TRUE;
				
					$permissions['User_Controller_login_html']=TRUE;
				
					$permissions['User_Controller_logged']=TRUE;
				
					$permissions['User_Controller_complete']=TRUE;
				
					$permissions['User_Controller_get_profile']=TRUE;
				
					$permissions['User_Controller_password_recover']=TRUE;
				
					$permissions['User_Controller_get_user_operator_list']=TRUE;
				
					$permissions['User_Controller_update_production_site']=TRUE;
				
					$permissions['User_Controller_set_param']=TRUE;
				
					$permissions['User_Controller_get_param']=TRUE;
				
					$permissions['User_Controller_select_login_role']=TRUE;
				
					$permissions['Constant_Controller_set_value']=TRUE;
				
					$permissions['Constant_Controller_get_list']=TRUE;
				
					$permissions['Constant_Controller_get_object']=TRUE;
				
					$permissions['Constant_Controller_get_values']=TRUE;
				
					$permissions['SessionVar_Controller_set_value']=TRUE;
				
					$permissions['SessionVar_Controller_get_values']=TRUE;
				
					$permissions['Enum_Controller_get_enum_list']=TRUE;
				
					$permissions['MainMenuConstructor_Controller_insert']=TRUE;
				
					$permissions['MainMenuConstructor_Controller_update']=TRUE;
				
					$permissions['MainMenuConstructor_Controller_delete']=TRUE;
				
					$permissions['MainMenuConstructor_Controller_get_list']=TRUE;
				
					$permissions['MainMenuConstructor_Controller_get_object']=TRUE;
				
					$permissions['MainMenuContent_Controller_insert']=TRUE;
				
					$permissions['MainMenuContent_Controller_update']=TRUE;
				
					$permissions['MainMenuContent_Controller_delete']=TRUE;
				
					$permissions['MainMenuContent_Controller_get_list']=TRUE;
				
					$permissions['MainMenuContent_Controller_get_object']=TRUE;
				
					$permissions['View_Controller_get_list']=TRUE;
				
					$permissions['View_Controller_complete']=TRUE;
				
					$permissions['View_Controller_get_section_list']=TRUE;
				
					$permissions['VariantStorage_Controller_insert']=TRUE;
				
					$permissions['VariantStorage_Controller_upsert_filter_data']=TRUE;
				
					$permissions['VariantStorage_Controller_upsert_col_visib_data']=TRUE;
				
					$permissions['VariantStorage_Controller_upsert_col_order_data']=TRUE;
				
					$permissions['VariantStorage_Controller_upsert_all_data']=TRUE;
				
					$permissions['VariantStorage_Controller_update']=TRUE;
				
					$permissions['VariantStorage_Controller_delete']=TRUE;
				
					$permissions['VariantStorage_Controller_get_list']=TRUE;
				
					$permissions['VariantStorage_Controller_get_object']=TRUE;
				
					$permissions['VariantStorage_Controller_get_filter_data']=TRUE;
				
					$permissions['VariantStorage_Controller_get_col_visib_data']=TRUE;
				
					$permissions['VariantStorage_Controller_get_col_order_data']=TRUE;
				
					$permissions['VariantStorage_Controller_get_all_data']=TRUE;
				
					$permissions['Graph_Controller_get_plant_load']=TRUE;
				
					$permissions['Graph_Controller_make_plant_load']=TRUE;
				
					$permissions['Graph_Controller_clear_cache']=TRUE;
				
					$permissions['Supplier_Controller_insert']=TRUE;
				
					$permissions['Supplier_Controller_update']=TRUE;
				
					$permissions['Supplier_Controller_delete']=TRUE;
				
					$permissions['Supplier_Controller_get_list']=TRUE;
				
					$permissions['Supplier_Controller_get_object']=TRUE;
				
					$permissions['Supplier_Controller_complete']=TRUE;
				
					$permissions['RawMaterialProcurUpload_Controller_get_list']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_insert']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_update']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_delete']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_get_list']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_get_object']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_before_open']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_get_actions']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_get_print']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_get_material_list']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_get_shift_list']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_complete_driver']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_complete_vehicle_plate']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_complete_store']=TRUE;
				
					$permissions['DOCMaterialProcurement_Controller_complete_sender_name']=TRUE;
				
					$permissions['DOCMaterialInventory_Controller_insert']=TRUE;
				
					$permissions['DOCMaterialInventory_Controller_update']=TRUE;
				
					$permissions['DOCMaterialInventory_Controller_delete']=TRUE;
				
					$permissions['DOCMaterialInventory_Controller_get_list']=TRUE;
				
					$permissions['DOCMaterialInventory_Controller_get_object']=TRUE;
				
					$permissions['DOCMaterialInventory_Controller_before_open']=TRUE;
				
					$permissions['DOCMaterialInventory_Controller_get_actions']=TRUE;
				
					$permissions['DOCMaterialInventory_Controller_get_print']=TRUE;
				
					$permissions['DOCMaterialInventory_Controller_get_details']=TRUE;
				
					$permissions['DOCMaterialInventoryDOCTMaterial_Controller_insert']=TRUE;
				
					$permissions['DOCMaterialInventoryDOCTMaterial_Controller_update']=TRUE;
				
					$permissions['DOCMaterialInventoryDOCTMaterial_Controller_delete']=TRUE;
				
					$permissions['DOCMaterialInventoryDOCTMaterial_Controller_get_list']=TRUE;
				
					$permissions['DOCMaterialInventoryDOCTMaterial_Controller_get_object']=TRUE;
				
					$permissions['Shift_Controller_get_open_shift']=TRUE;
				
					$permissions['Shift_Controller_close_shift']=TRUE;
				
					$permissions['Lang_Controller_insert']=TRUE;
				
					$permissions['Lang_Controller_update']=TRUE;
				
					$permissions['Lang_Controller_delete']=TRUE;
				
					$permissions['Lang_Controller_get_list']=TRUE;
				
					$permissions['Lang_Controller_get_object']=TRUE;
				
					$permissions['Lang_Controller_complete']=TRUE;
				
					$permissions['SMSPattern_Controller_insert']=TRUE;
				
					$permissions['SMSPattern_Controller_update']=TRUE;
				
					$permissions['SMSPattern_Controller_delete']=TRUE;
				
					$permissions['SMSPattern_Controller_get_list']=TRUE;
				
					$permissions['SMSPattern_Controller_get_object']=TRUE;
				
					$permissions['SMSPatternUserPhone_Controller_insert']=TRUE;
				
					$permissions['SMSPatternUserPhone_Controller_update']=TRUE;
				
					$permissions['SMSPatternUserPhone_Controller_delete']=TRUE;
				
					$permissions['SMSPatternUserPhone_Controller_get_list']=TRUE;
				
					$permissions['SMSPatternUserPhone_Controller_get_object']=TRUE;
				
					$permissions['LabData_Controller_insert']=TRUE;
				
					$permissions['LabData_Controller_update']=TRUE;
				
					$permissions['LabData_Controller_delete']=TRUE;
				
					$permissions['LabData_Controller_get_list']=TRUE;
				
					$permissions['LabData_Controller_get_object']=TRUE;
				
					$permissions['LabEntry_Controller_insert']=TRUE;
				
					$permissions['LabEntry_Controller_update']=TRUE;
				
					$permissions['LabEntry_Controller_delete']=TRUE;
				
					$permissions['LabEntry_Controller_get_list']=TRUE;
				
					$permissions['LabEntry_Controller_get_object']=TRUE;
				
					$permissions['LabEntry_Controller_lab_report']=TRUE;
				
					$permissions['LabEntry_Controller_item_report']=TRUE;
				
					$permissions['LabEntry_Controller_lab_avg_report']=TRUE;
				
					$permissions['LabEntry_Controller_item_on_rate_period_report']=TRUE;
				
					$permissions['LabEntry_Controller_lab_entry30_days']=TRUE;
				
					$permissions['LabEntry_Controller_lab_vehicle_list']=TRUE;
				
					$permissions['LabEntry_Controller_lab_orders_list']=TRUE;
				
					$permissions['LabEntry_Controller_chart_rep_init']=TRUE;
				
					$permissions['LabEntryDetail_Controller_insert']=TRUE;
				
					$permissions['LabEntryDetail_Controller_update']=TRUE;
				
					$permissions['LabEntryDetail_Controller_delete']=TRUE;
				
					$permissions['LabEntryDetail_Controller_get_list']=TRUE;
				
					$permissions['LabEntryDetail_Controller_get_object']=TRUE;
				
					$permissions['OrderFromClient_Controller_delete']=TRUE;
				
					$permissions['OrderFromClient_Controller_get_list']=TRUE;
				
					$permissions['OrderFromClient_Controller_set_viewed']=TRUE;
				
					$permissions['Caller_Controller_call']=TRUE;
				
					$permissions['RawMaterialStoreUserData_Controller_insert']=TRUE;
				
					$permissions['RawMaterialStoreUserData_Controller_update']=TRUE;
				
					$permissions['RawMaterialStoreUserData_Controller_delete']=TRUE;
				
					$permissions['RawMaterialStoreUserData_Controller_get_list']=TRUE;
				
					$permissions['RawMaterialStoreUserData_Controller_get_object']=TRUE;
				
					$permissions['Employee_Controller_insert']=TRUE;
				
					$permissions['Employee_Controller_update']=TRUE;
				
					$permissions['Employee_Controller_delete']=TRUE;
				
					$permissions['Employee_Controller_get_list']=TRUE;
				
					$permissions['Employee_Controller_get_object']=TRUE;
				
					$permissions['Employee_Controller_set_work_schedule_hour']=TRUE;
				
					$permissions['EmployeeWorkTimeSchedule_Controller_insert']=TRUE;
				
					$permissions['EmployeeWorkTimeSchedule_Controller_update']=TRUE;
				
					$permissions['EmployeeWorkTimeSchedule_Controller_delete']=TRUE;
				
					$permissions['EmployeeWorkTimeSchedule_Controller_get_list']=TRUE;
				
					$permissions['AstCall_Controller_update']=TRUE;
				
					$permissions['AstCall_Controller_get_list']=TRUE;
				
					$permissions['AstCall_Controller_get_object']=TRUE;
				
					$permissions['AstCall_Controller_client_call_hist']=TRUE;
				
					$permissions['AstCall_Controller_client_ship_hist']=TRUE;
				
					$permissions['AstCall_Controller_active_call']=TRUE;
				
					$permissions['AstCall_Controller_active_call_inform']=TRUE;
				
					$permissions['AstCall_Controller_set_active_call_client_kind']=TRUE;
				
					$permissions['AstCall_Controller_new_client']=TRUE;
				
					$permissions['AstCall_Controller_manager_report']=TRUE;
				
					$permissions['AstCall_Controller_restart_ast']=TRUE;
				
					$permissions['UserMacAddress_Controller_insert']=TRUE;
				
					$permissions['UserMacAddress_Controller_update']=TRUE;
				
					$permissions['UserMacAddress_Controller_delete']=TRUE;
				
					$permissions['UserMacAddress_Controller_get_list']=TRUE;
				
					$permissions['UserMacAddress_Controller_get_object']=TRUE;
				
					$permissions['ClientTel_Controller_insert']=TRUE;
				
					$permissions['ClientTel_Controller_update']=TRUE;
				
					$permissions['ClientTel_Controller_delete']=TRUE;
				
					$permissions['ClientTel_Controller_get_list']=TRUE;
				
					$permissions['ClientTel_Controller_get_object']=TRUE;
				
					$permissions['ClientTel_Controller_complete']=TRUE;
				
					$permissions['ClientTel_Controller_complete_tel']=TRUE;
				
					$permissions['ClientTel_Controller_upsert']=TRUE;
				
					$permissions['ClientTel_Controller_get_ref']=TRUE;
				
					$permissions['ClientType_Controller_insert']=TRUE;
				
					$permissions['ClientType_Controller_update']=TRUE;
				
					$permissions['ClientType_Controller_delete']=TRUE;
				
					$permissions['ClientType_Controller_get_list']=TRUE;
				
					$permissions['ClientType_Controller_get_object']=TRUE;
				
					$permissions['ClientComeFrom_Controller_insert']=TRUE;
				
					$permissions['ClientComeFrom_Controller_update']=TRUE;
				
					$permissions['ClientComeFrom_Controller_delete']=TRUE;
				
					$permissions['ClientComeFrom_Controller_get_list']=TRUE;
				
					$permissions['ClientComeFrom_Controller_get_object']=TRUE;
				
					$permissions['PumpVehicle_Controller_insert']=TRUE;
				
					$permissions['PumpVehicle_Controller_update']=TRUE;
				
					$permissions['PumpVehicle_Controller_delete']=TRUE;
				
					$permissions['PumpVehicle_Controller_get_list']=TRUE;
				
					$permissions['PumpVehicle_Controller_get_work_list']=TRUE;
				
					$permissions['PumpVehicle_Controller_get_object']=TRUE;
				
					$permissions['PumpVehicle_Controller_get_price']=TRUE;
				
					$permissions['PumpVehicle_Controller_get_contact_refs']=TRUE;
				
					$permissions['PumpPrice_Controller_insert']=TRUE;
				
					$permissions['PumpPrice_Controller_update']=TRUE;
				
					$permissions['PumpPrice_Controller_delete']=TRUE;
				
					$permissions['PumpPrice_Controller_get_list']=TRUE;
				
					$permissions['PumpPrice_Controller_get_object']=TRUE;
				
					$permissions['PumpPriceValue_Controller_insert']=TRUE;
				
					$permissions['PumpPriceValue_Controller_update']=TRUE;
				
					$permissions['PumpPriceValue_Controller_delete']=TRUE;
				
					$permissions['PumpPriceValue_Controller_get_list']=TRUE;
				
					$permissions['PumpPriceValue_Controller_get_object']=TRUE;
				
					$permissions['Quarry_Controller_insert']=TRUE;
				
					$permissions['Quarry_Controller_update']=TRUE;
				
					$permissions['Quarry_Controller_delete']=TRUE;
				
					$permissions['Quarry_Controller_get_list']=TRUE;
				
					$permissions['Quarry_Controller_get_object']=TRUE;
				
					$permissions['Quarry_Controller_complete']=TRUE;
				
					$permissions['SandQuarryVal_Controller_insert']=TRUE;
				
					$permissions['SandQuarryVal_Controller_update']=TRUE;
				
					$permissions['SandQuarryVal_Controller_delete']=TRUE;
				
					$permissions['SandQuarryVal_Controller_get_list']=TRUE;
				
					$permissions['SandQuarryVal_Controller_get_object']=TRUE;
				
					$permissions['StoneQuarryVal_Controller_insert']=TRUE;
				
					$permissions['StoneQuarryVal_Controller_update']=TRUE;
				
					$permissions['StoneQuarryVal_Controller_delete']=TRUE;
				
					$permissions['StoneQuarryVal_Controller_get_list']=TRUE;
				
					$permissions['StoneQuarryVal_Controller_get_object']=TRUE;
				
					$permissions['SpecialistRequest_Controller_insert']=TRUE;
				
					$permissions['SpecialistRequest_Controller_update']=TRUE;
				
					$permissions['SpecialistRequest_Controller_delete']=TRUE;
				
					$permissions['SpecialistRequest_Controller_get_list']=TRUE;
				
					$permissions['SpecialistRequest_Controller_get_object']=TRUE;
				
					$permissions['SpecialistRequest_Controller_set_viewed']=TRUE;
				
					$permissions['SiteFeedBack_Controller_get_list']=TRUE;
				
					$permissions['SiteFeedBack_Controller_set_viewed']=TRUE;
				
					$permissions['EmailTemplate_Controller_insert']=TRUE;
				
					$permissions['EmailTemplate_Controller_update']=TRUE;
				
					$permissions['EmailTemplate_Controller_delete']=TRUE;
				
					$permissions['EmailTemplate_Controller_get_list']=TRUE;
				
					$permissions['EmailTemplate_Controller_get_object']=TRUE;
				
					$permissions['ClientValidDuplicate_Controller_delete']=TRUE;
				
					$permissions['ClientValidDuplicate_Controller_get_list']=TRUE;
				
					$permissions['OrderPump_Controller_insert']=TRUE;
				
					$permissions['OrderPump_Controller_update']=TRUE;
				
					$permissions['OrderPump_Controller_delete']=TRUE;
				
					$permissions['OrderPump_Controller_get_list']=TRUE;
				
					$permissions['OrderPump_Controller_get_object']=TRUE;
				
					$permissions['ShipmentTimeNorm_Controller_insert']=TRUE;
				
					$permissions['ShipmentTimeNorm_Controller_update']=TRUE;
				
					$permissions['ShipmentTimeNorm_Controller_delete']=TRUE;
				
					$permissions['ShipmentTimeNorm_Controller_get_list']=TRUE;
				
					$permissions['ShipmentTimeNorm_Controller_get_object']=TRUE;
				
					$permissions['MainMenu_Controller_get_list']=TRUE;
				
					$permissions['About_Controller_get_object']=TRUE;
				
					$permissions['Captcha_Controller_get']=TRUE;
				
					$permissions['ProductionBase_Controller_insert']=TRUE;
				
					$permissions['ProductionBase_Controller_update']=TRUE;
				
					$permissions['ProductionBase_Controller_delete']=TRUE;
				
					$permissions['ProductionBase_Controller_get_list']=TRUE;
				
					$permissions['ProductionBase_Controller_get_object']=TRUE;
				
					$permissions['ProductionSite_Controller_insert']=TRUE;
				
					$permissions['ProductionSite_Controller_update']=TRUE;
				
					$permissions['ProductionSite_Controller_delete']=TRUE;
				
					$permissions['ProductionSite_Controller_get_list']=TRUE;
				
					$permissions['ProductionSite_Controller_get_object']=TRUE;
				
					$permissions['ProductionSite_Controller_get_list_for_edit']=TRUE;
				
					$permissions['ShipQuantForCostGrade_Controller_insert']=TRUE;
				
					$permissions['ShipQuantForCostGrade_Controller_update']=TRUE;
				
					$permissions['ShipQuantForCostGrade_Controller_delete']=TRUE;
				
					$permissions['ShipQuantForCostGrade_Controller_get_list']=TRUE;
				
					$permissions['ShipQuantForCostGrade_Controller_get_object']=TRUE;
				
					$permissions['SMSForSending_Controller_get_list']=TRUE;
				
					$permissions['TrackerZoneControl_Controller_insert']=TRUE;
				
					$permissions['TrackerZoneControl_Controller_update']=TRUE;
				
					$permissions['TrackerZoneControl_Controller_delete']=TRUE;
				
					$permissions['TrackerZoneControl_Controller_get_list']=TRUE;
				
					$permissions['TrackerZoneControl_Controller_get_object']=TRUE;
				
					$permissions['Weather_Controller_get_current']=TRUE;
				
					$permissions['VehicleDriverForSchedGen_Controller_insert']=TRUE;
				
					$permissions['VehicleDriverForSchedGen_Controller_update']=TRUE;
				
					$permissions['VehicleDriverForSchedGen_Controller_delete']=TRUE;
				
					$permissions['VehicleDriverForSchedGen_Controller_get_list']=TRUE;
				
					$permissions['VehicleDriverForSchedGen_Controller_get_object']=TRUE;
				
					$permissions['ShipmentForDriverCostHeader_Controller_insert']=TRUE;
				
					$permissions['ShipmentForDriverCostHeader_Controller_update']=TRUE;
				
					$permissions['ShipmentForDriverCostHeader_Controller_delete']=TRUE;
				
					$permissions['ShipmentForDriverCostHeader_Controller_get_list']=TRUE;
				
					$permissions['ShipmentForDriverCostHeader_Controller_get_object']=TRUE;
				
					$permissions['ShipmentForDriverCost_Controller_insert']=TRUE;
				
					$permissions['ShipmentForDriverCost_Controller_update']=TRUE;
				
					$permissions['ShipmentForDriverCost_Controller_delete']=TRUE;
				
					$permissions['ShipmentForDriverCost_Controller_get_list']=TRUE;
				
					$permissions['ShipmentForDriverCost_Controller_get_object']=TRUE;
				
					$permissions['ShipmentForOwnerCostHeader_Controller_insert']=TRUE;
				
					$permissions['ShipmentForOwnerCostHeader_Controller_update']=TRUE;
				
					$permissions['ShipmentForOwnerCostHeader_Controller_delete']=TRUE;
				
					$permissions['ShipmentForOwnerCostHeader_Controller_get_list']=TRUE;
				
					$permissions['ShipmentForOwnerCostHeader_Controller_get_object']=TRUE;
				
					$permissions['ShipmentForOwnerCost_Controller_insert']=TRUE;
				
					$permissions['ShipmentForOwnerCost_Controller_update']=TRUE;
				
					$permissions['ShipmentForOwnerCost_Controller_delete']=TRUE;
				
					$permissions['ShipmentForOwnerCost_Controller_get_list']=TRUE;
				
					$permissions['ShipmentForOwnerCost_Controller_get_object']=TRUE;
				
					$permissions['ConcreteCostHeader_Controller_insert']=TRUE;
				
					$permissions['ConcreteCostHeader_Controller_update']=TRUE;
				
					$permissions['ConcreteCostHeader_Controller_delete']=TRUE;
				
					$permissions['ConcreteCostHeader_Controller_get_list']=TRUE;
				
					$permissions['ConcreteCostHeader_Controller_get_object']=TRUE;
				
					$permissions['ConcreteCostHeader_Controller_get_client_price_list']=TRUE;
				
					$permissions['ConcreteCost_Controller_insert']=TRUE;
				
					$permissions['ConcreteCost_Controller_update']=TRUE;
				
					$permissions['ConcreteCost_Controller_delete']=TRUE;
				
					$permissions['ConcreteCost_Controller_get_list']=TRUE;
				
					$permissions['ConcreteCost_Controller_get_object']=TRUE;
				
					$permissions['ConcreteCostForOwnerHeader_Controller_insert']=TRUE;
				
					$permissions['ConcreteCostForOwnerHeader_Controller_update']=TRUE;
				
					$permissions['ConcreteCostForOwnerHeader_Controller_delete']=TRUE;
				
					$permissions['ConcreteCostForOwnerHeader_Controller_get_list']=TRUE;
				
					$permissions['ConcreteCostForOwnerHeader_Controller_get_object']=TRUE;
				
					$permissions['ConcreteCostForOwner_Controller_insert']=TRUE;
				
					$permissions['ConcreteCostForOwner_Controller_update']=TRUE;
				
					$permissions['ConcreteCostForOwner_Controller_delete']=TRUE;
				
					$permissions['ConcreteCostForOwner_Controller_get_list']=TRUE;
				
					$permissions['ConcreteCostForOwner_Controller_get_object']=TRUE;
				
					$permissions['VehicleOwner_Controller_insert']=TRUE;
				
					$permissions['VehicleOwner_Controller_update']=TRUE;
				
					$permissions['VehicleOwner_Controller_delete']=TRUE;
				
					$permissions['VehicleOwner_Controller_get_list']=TRUE;
				
					$permissions['VehicleOwner_Controller_get_object']=TRUE;
				
					$permissions['VehicleOwner_Controller_complete']=TRUE;
				
					$permissions['VehicleOwner_Controller_get_tot_report']=TRUE;
				
					$permissions['VehicleOwner_Controller_get_tot_income_report']=TRUE;
				
					$permissions['VehicleOwner_Controller_get_tot_income_report_all']=TRUE;
				
					$permissions['VehicleOwnerClient_Controller_insert']=TRUE;
				
					$permissions['VehicleOwnerClient_Controller_update']=TRUE;
				
					$permissions['VehicleOwnerClient_Controller_delete']=TRUE;
				
					$permissions['VehicleOwnerClient_Controller_get_list']=TRUE;
				
					$permissions['VehicleOwnerClient_Controller_get_object']=TRUE;
				
					$permissions['VehicleOwnerConcretePrice_Controller_insert']=TRUE;
				
					$permissions['VehicleOwnerConcretePrice_Controller_update']=TRUE;
				
					$permissions['VehicleOwnerConcretePrice_Controller_delete']=TRUE;
				
					$permissions['VehicleOwnerConcretePrice_Controller_get_list']=TRUE;
				
					$permissions['VehicleOwnerConcretePrice_Controller_get_object']=TRUE;
				
					$permissions['ShipmentCancelation_Controller_insert']=TRUE;
				
					$permissions['ShipmentCancelation_Controller_update']=TRUE;
				
					$permissions['ShipmentCancelation_Controller_delete']=TRUE;
				
					$permissions['ShipmentCancelation_Controller_get_list']=TRUE;
				
					$permissions['ShipmentCancelation_Controller_get_object']=TRUE;
				
					$permissions['TelList_Controller_insert']=TRUE;
				
					$permissions['TelList_Controller_update']=TRUE;
				
					$permissions['TelList_Controller_delete']=TRUE;
				
					$permissions['TelList_Controller_get_list']=TRUE;
				
					$permissions['TelList_Controller_get_object']=TRUE;
				
					$permissions['ConcreteTypeMapToProduction_Controller_insert']=TRUE;
				
					$permissions['ConcreteTypeMapToProduction_Controller_update']=TRUE;
				
					$permissions['ConcreteTypeMapToProduction_Controller_delete']=TRUE;
				
					$permissions['ConcreteTypeMapToProduction_Controller_get_list']=TRUE;
				
					$permissions['ConcreteTypeMapToProduction_Controller_get_object']=TRUE;
				
					$permissions['UserMapToProduction_Controller_insert']=TRUE;
				
					$permissions['UserMapToProduction_Controller_update']=TRUE;
				
					$permissions['UserMapToProduction_Controller_delete']=TRUE;
				
					$permissions['UserMapToProduction_Controller_get_list']=TRUE;
				
					$permissions['UserMapToProduction_Controller_get_object']=TRUE;
				
					$permissions['RawMaterialMapToProduction_Controller_insert']=TRUE;
				
					$permissions['RawMaterialMapToProduction_Controller_update']=TRUE;
				
					$permissions['RawMaterialMapToProduction_Controller_delete']=TRUE;
				
					$permissions['RawMaterialMapToProduction_Controller_get_list']=TRUE;
				
					$permissions['RawMaterialMapToProduction_Controller_get_object']=TRUE;
				
					$permissions['MaterialFactConsumption_Controller_insert']=TRUE;
				
					$permissions['MaterialFactConsumption_Controller_update']=TRUE;
				
					$permissions['MaterialFactConsumption_Controller_delete']=TRUE;
				
					$permissions['MaterialFactConsumption_Controller_get_list']=TRUE;
				
					$permissions['MaterialFactConsumption_Controller_get_rolled_list']=TRUE;
				
					$permissions['MaterialFactConsumption_Controller_get_object']=TRUE;
				
					$permissions['MaterialFactConsumption_Controller_upload_production_file']=TRUE;
				
					$permissions['MaterialFactConsumption_Controller_upload_production_file_auto']=TRUE;
				
					$permissions['MaterialFactConsumption_Controller_get_report']=TRUE;
				
					$permissions['MaterialFactConsumption_Controller_delete_material']=TRUE;
				
					$permissions['MaterialFactConsumptionCorretion_Controller_insert']=TRUE;
				
					$permissions['MaterialFactConsumptionCorretion_Controller_update']=TRUE;
				
					$permissions['MaterialFactConsumptionCorretion_Controller_delete']=TRUE;
				
					$permissions['MaterialFactConsumptionCorretion_Controller_get_list']=TRUE;
				
					$permissions['MaterialFactConsumptionCorretion_Controller_get_object']=TRUE;
				
					$permissions['MaterialFactConsumptionCorretion_Controller_operator_insert_correction']=TRUE;
				
					$permissions['MaterialFactConsumptionCorretion_Controller_operator_add_material_to_production']=TRUE;
				
					$permissions['CementSilo_Controller_insert']=TRUE;
				
					$permissions['CementSilo_Controller_update']=TRUE;
				
					$permissions['CementSilo_Controller_delete']=TRUE;
				
					$permissions['CementSilo_Controller_get_list']=TRUE;
				
					$permissions['CementSilo_Controller_get_object']=TRUE;
				
					$permissions['CementSilo_Controller_reset_balance']=TRUE;
				
					$permissions['CementSiloBalanceReset_Controller_insert']=TRUE;
				
					$permissions['CementSiloBalanceReset_Controller_update']=TRUE;
				
					$permissions['CementSiloBalanceReset_Controller_delete']=TRUE;
				
					$permissions['CementSiloBalanceReset_Controller_get_list']=TRUE;
				
					$permissions['CementSiloBalanceReset_Controller_get_object']=TRUE;
				
					$permissions['OwnerList_Controller_insert']=TRUE;
				
					$permissions['OwnerList_Controller_update']=TRUE;
				
					$permissions['OwnerList_Controller_delete']=TRUE;
				
					$permissions['OwnerList_Controller_get_list']=TRUE;
				
					$permissions['OwnerList_Controller_get_object']=TRUE;
				
					$permissions['ClientLocalList_Controller_insert']=TRUE;
				
					$permissions['ClientLocalList_Controller_update']=TRUE;
				
					$permissions['ClientLocalList_Controller_delete']=TRUE;
				
					$permissions['ClientLocalList_Controller_get_list']=TRUE;
				
					$permissions['ClientLocalList_Controller_get_object']=TRUE;
				
					$permissions['PumpVehiclePriceList_Controller_insert']=TRUE;
				
					$permissions['PumpVehiclePriceList_Controller_update']=TRUE;
				
					$permissions['PumpVehiclePriceList_Controller_delete']=TRUE;
				
					$permissions['PumpVehiclePriceList_Controller_get_list']=TRUE;
				
					$permissions['PumpVehiclePriceList_Controller_get_object']=TRUE;
				
					$permissions['ELKONLog_Controller_get_list']=TRUE;
				
					$permissions['ELKONLog_Controller_get_object']=TRUE;
				
					$permissions['Production_Controller_insert']=TRUE;
				
					$permissions['Production_Controller_update']=TRUE;
				
					$permissions['Production_Controller_delete']=TRUE;
				
					$permissions['Production_Controller_get_list']=TRUE;
				
					$permissions['Production_Controller_get_object']=TRUE;
				
					$permissions['Production_Controller_check_data']=TRUE;
				
					$permissions['Production_Controller_check_production']=TRUE;
				
					$permissions['Production_Controller_get_production_material_list']=TRUE;
				
					$permissions['Production_Controller_get_ship_prod_quant_dif']=TRUE;
				
					$permissions['MaterialFactBalanceCorretion_Controller_insert']=TRUE;
				
					$permissions['MaterialFactBalanceCorretion_Controller_update']=TRUE;
				
					$permissions['MaterialFactBalanceCorretion_Controller_delete']=TRUE;
				
					$permissions['MaterialFactBalanceCorretion_Controller_get_list']=TRUE;
				
					$permissions['MaterialFactBalanceCorretion_Controller_get_object']=TRUE;
				
					$permissions['ProductionVehicleCorrection_Controller_insert']=TRUE;
				
					$permissions['ProductionVehicleCorrection_Controller_update']=TRUE;
				
					$permissions['ProductionVehicleCorrection_Controller_delete']=TRUE;
				
					$permissions['ProductionVehicleCorrection_Controller_get_list']=TRUE;
				
					$permissions['ProductionVehicleCorrection_Controller_get_object']=TRUE;
				
					$permissions['ProductionComment_Controller_insert']=TRUE;
				
					$permissions['ProductionComment_Controller_update']=TRUE;
				
					$permissions['ProductionComment_Controller_delete']=TRUE;
				
					$permissions['ProductionComment_Controller_get_list']=TRUE;
				
					$permissions['ProductionComment_Controller_get_object']=TRUE;
				
					$permissions['GPSTracker_Controller_insert']=TRUE;
				
					$permissions['GPSTracker_Controller_update']=TRUE;
				
					$permissions['GPSTracker_Controller_delete']=TRUE;
				
					$permissions['GPSTracker_Controller_get_list']=TRUE;
				
					$permissions['GPSTracker_Controller_get_object']=TRUE;
				
					$permissions['GPSTracker_Controller_complete']=TRUE;
				
					$permissions['StoreMapToProductionSite_Controller_insert']=TRUE;
				
					$permissions['StoreMapToProductionSite_Controller_update']=TRUE;
				
					$permissions['StoreMapToProductionSite_Controller_delete']=TRUE;
				
					$permissions['StoreMapToProductionSite_Controller_get_list']=TRUE;
				
					$permissions['StoreMapToProductionSite_Controller_get_object']=TRUE;
				
					$permissions['GornyiCarrierMatch_Controller_insert']=TRUE;
				
					$permissions['GornyiCarrierMatch_Controller_update']=TRUE;
				
					$permissions['GornyiCarrierMatch_Controller_delete']=TRUE;
				
					$permissions['GornyiCarrierMatch_Controller_get_list']=TRUE;
				
					$permissions['GornyiCarrierMatch_Controller_get_object']=TRUE;
				
					$permissions['RoleViewRestriction_Controller_insert']=TRUE;
				
					$permissions['RoleViewRestriction_Controller_update']=TRUE;
				
					$permissions['RoleViewRestriction_Controller_delete']=TRUE;
				
					$permissions['RoleViewRestriction_Controller_get_list']=TRUE;
				
					$permissions['RoleViewRestriction_Controller_get_object']=TRUE;
				
					$permissions['ExcelTemplate_Controller_insert']=TRUE;
				
					$permissions['ExcelTemplate_Controller_update']=TRUE;
				
					$permissions['ExcelTemplate_Controller_delete']=TRUE;
				
					$permissions['ExcelTemplate_Controller_get_list']=TRUE;
				
					$permissions['ExcelTemplate_Controller_get_object']=TRUE;
				
					$permissions['ExcelTemplate_Controller_download_excel_file']=TRUE;
				
					$permissions['ExcelTemplate_Controller_delete_excel_file']=TRUE;
				
					$permissions['ExcelTemplateFieldMatch_Controller_insert']=TRUE;
				
					$permissions['ExcelTemplateFieldMatch_Controller_update']=TRUE;
				
					$permissions['ExcelTemplateFieldMatch_Controller_delete']=TRUE;
				
					$permissions['ExcelTemplateFieldMatch_Controller_get_list']=TRUE;
				
					$permissions['ExcelTemplateFieldMatch_Controller_get_object']=TRUE;
				
					$permissions['Bank_Controller_get_list']=TRUE;
				
					$permissions['Bank_Controller_get_object']=TRUE;
				
					$permissions['Bank_Controller_complete']=TRUE;
				
					$permissions['PeriodValue_Controller_insert']=TRUE;
				
					$permissions['PeriodValue_Controller_update']=TRUE;
				
					$permissions['PeriodValue_Controller_delete']=TRUE;
				
					$permissions['PeriodValue_Controller_get_list']=TRUE;
				
					$permissions['PeriodValue_Controller_get_object']=TRUE;
				
					$permissions['PeriodValue_Controller_get_last_val']=TRUE;
				
					$permissions['TmUser_Controller_delete']=TRUE;
				
					$permissions['TmUser_Controller_get_list']=TRUE;
				
					$permissions['TmUser_Controller_get_object']=TRUE;
				
					$permissions['TmUser_Controller_get_full_photo']=TRUE;
				
					$permissions['TmUser_Controller_complete']=TRUE;
				
					$permissions['TmUser_Controller_update']=TRUE;
				
					$permissions['TmOutMessage_Controller_get_list']=TRUE;
				
					$permissions['TmOutMessage_Controller_get_object']=TRUE;
				
					$permissions['TmOutMessage_Controller_send']=TRUE;
				
					$permissions['TmOutMessage_Controller_send_media']=TRUE;
				
					$permissions['TmOutMessage_Controller_tm_invite']=TRUE;
				
					$permissions['TmOutMessage_Controller_tm_invite_contact']=TRUE;
				
					$permissions['TmOutMessage_Controller_get_recipient_inf']=TRUE;
				
					$permissions['TmInMessage_Controller_get_list']=TRUE;
				
					$permissions['TmInMessage_Controller_get_object']=TRUE;
				
					$permissions['Chat_Controller_get_history']=TRUE;
				
					$permissions['Post_Controller_insert']=TRUE;
				
					$permissions['Post_Controller_update']=TRUE;
				
					$permissions['Post_Controller_delete']=TRUE;
				
					$permissions['Post_Controller_get_list']=TRUE;
				
					$permissions['Post_Controller_get_object']=TRUE;
				
					$permissions['Post_Controller_complete']=TRUE;
				
					$permissions['Contact_Controller_insert']=TRUE;
				
					$permissions['Contact_Controller_update']=TRUE;
				
					$permissions['Contact_Controller_delete']=TRUE;
				
					$permissions['Contact_Controller_get_list']=TRUE;
				
					$permissions['Contact_Controller_get_object']=TRUE;
				
					$permissions['Contact_Controller_complete']=TRUE;
				
					$permissions['Contact_Controller_complete_tm']=TRUE;
				
					$permissions['Contact_Controller_upsert']=TRUE;
				
					$permissions['EntityContact_Controller_insert']=TRUE;
				
					$permissions['EntityContact_Controller_update']=TRUE;
				
					$permissions['EntityContact_Controller_delete']=TRUE;
				
					$permissions['EntityContact_Controller_get_list']=TRUE;
				
					$permissions['EntityContact_Controller_get_object']=TRUE;
				
					$permissions['RawMaterialTicket_Controller_insert']=TRUE;
				
					$permissions['RawMaterialTicket_Controller_update']=TRUE;
				
					$permissions['RawMaterialTicket_Controller_delete']=TRUE;
				
					$permissions['RawMaterialTicket_Controller_get_list']=TRUE;
				
					$permissions['RawMaterialTicket_Controller_get_object']=TRUE;
				
					$permissions['RawMaterialTicket_Controller_generate']=TRUE;
				
					$permissions['RawMaterialTicket_Controller_close_ticket']=TRUE;
				
					$permissions['RawMaterialTicket_Controller_get_carrier_agg_list']=TRUE;
				
					$permissions['LabChart_Controller_get_lab_chart1']=TRUE;
				
					$permissions['LabChart_Controller_get_lab_chart2']=TRUE;
				
					$permissions['ClientSpecification_Controller_insert']=TRUE;
				
					$permissions['ClientSpecification_Controller_update']=TRUE;
				
					$permissions['ClientSpecification_Controller_delete']=TRUE;
				
					$permissions['ClientSpecification_Controller_get_list']=TRUE;
				
					$permissions['ClientSpecification_Controller_get_object']=TRUE;
				
					$permissions['ClientSpecification_Controller_complete_for_client']=TRUE;
				
					$permissions['ClientSpecificationFlow_Controller_get_list']=TRUE;
				
					$permissions['DestinationProdBasePrice_Controller_insert']=TRUE;
				
					$permissions['DestinationProdBasePrice_Controller_update']=TRUE;
				
					$permissions['DestinationProdBasePrice_Controller_delete']=TRUE;
				
					$permissions['DestinationProdBasePrice_Controller_get_list']=TRUE;
				
					$permissions['DestinationProdBasePrice_Controller_get_object']=TRUE;
				
					$permissions['DestinationProdBaseDriverPrice_Controller_insert']=TRUE;
				
					$permissions['DestinationProdBaseDriverPrice_Controller_update']=TRUE;
				
					$permissions['DestinationProdBaseDriverPrice_Controller_delete']=TRUE;
				
					$permissions['DestinationProdBaseDriverPrice_Controller_get_list']=TRUE;
				
					$permissions['DestinationProdBaseDriverPrice_Controller_get_object']=TRUE;
				
					$permissions['UserOperation_Controller_insert']=TRUE;
				
					$permissions['UserOperation_Controller_update']=TRUE;
				
					$permissions['UserOperation_Controller_delete']=TRUE;
				
					$permissions['UserOperation_Controller_get_list']=TRUE;
				
					$permissions['UserOperation_Controller_get_object']=TRUE;
				
					$permissions['VehicleTotRepItem_Controller_insert']=TRUE;
				
					$permissions['VehicleTotRepItem_Controller_update']=TRUE;
				
					$permissions['VehicleTotRepItem_Controller_delete']=TRUE;
				
					$permissions['VehicleTotRepItem_Controller_get_list']=TRUE;
				
					$permissions['VehicleTotRepItem_Controller_get_object']=TRUE;
				
					$permissions['VehicleTotRepCommonItem_Controller_insert']=TRUE;
				
					$permissions['VehicleTotRepCommonItem_Controller_update']=TRUE;
				
					$permissions['VehicleTotRepCommonItem_Controller_delete']=TRUE;
				
					$permissions['VehicleTotRepCommonItem_Controller_get_list']=TRUE;
				
					$permissions['VehicleTotRepCommonItem_Controller_get_object']=TRUE;
				
					$permissions['VehicleTotRepItemValue_Controller_insert']=TRUE;
				
					$permissions['VehicleTotRepItemValue_Controller_update']=TRUE;
				
					$permissions['VehicleTotRepItemValue_Controller_delete']=TRUE;
				
					$permissions['VehicleTotRepItemValue_Controller_get_list']=TRUE;
				
					$permissions['VehicleTotRepItemValue_Controller_get_object']=TRUE;
				
					$permissions['VehicleTotRepItemValue_Controller_update_values']=TRUE;
				
					$permissions['VehicleTotRepCommonItemValue_Controller_insert']=TRUE;
				
					$permissions['VehicleTotRepCommonItemValue_Controller_update']=TRUE;
				
					$permissions['VehicleTotRepCommonItemValue_Controller_delete']=TRUE;
				
					$permissions['VehicleTotRepCommonItemValue_Controller_get_list']=TRUE;
				
					$permissions['VehicleTotRepCommonItemValue_Controller_get_object']=TRUE;
				
					$permissions['VehicleTotRepCommonItemValue_Controller_update_values']=TRUE;
				
					$permissions['VehicleTotRepBalance_Controller_insert']=TRUE;
				
					$permissions['VehicleTotRepBalance_Controller_update']=TRUE;
				
					$permissions['VehicleTotRepBalance_Controller_delete']=TRUE;
				
					$permissions['VehicleTotRepBalance_Controller_get_list']=TRUE;
				
					$permissions['VehicleTotRepBalance_Controller_get_object']=TRUE;
				
					$permissions['TaskImportanceLevel_Controller_insert']=TRUE;
				
					$permissions['TaskImportanceLevel_Controller_update']=TRUE;
				
					$permissions['TaskImportanceLevel_Controller_delete']=TRUE;
				
					$permissions['TaskImportanceLevel_Controller_get_list']=TRUE;
				
					$permissions['TaskImportanceLevel_Controller_get_object']=TRUE;
				
					$permissions['Task_Controller_insert']=TRUE;
				
					$permissions['Task_Controller_update']=TRUE;
				
					$permissions['Task_Controller_delete']=TRUE;
				
					$permissions['Task_Controller_get_list']=TRUE;
				
					$permissions['Task_Controller_get_object']=TRUE;
				
					$permissions['TaskDetail_Controller_insert']=TRUE;
				
					$permissions['TaskDetail_Controller_update']=TRUE;
				
					$permissions['TaskDetail_Controller_delete']=TRUE;
				
					$permissions['TaskDetail_Controller_get_list']=TRUE;
				
					$permissions['TaskDetail_Controller_get_object']=TRUE;
				
					$permissions['Attachment_Controller_delete_file']=TRUE;
				
					$permissions['Attachment_Controller_get_file']=TRUE;
				
					$permissions['Attachment_Controller_add_file']=TRUE;
				
					$permissions['ProgUpdate_Controller_insert']=TRUE;
				
					$permissions['ProgUpdate_Controller_update']=TRUE;
				
					$permissions['ProgUpdate_Controller_delete']=TRUE;
				
					$permissions['ProgUpdate_Controller_get_list']=TRUE;
				
					$permissions['ProgUpdate_Controller_get_object']=TRUE;
				
					$permissions['ProgUpdateDetail_Controller_insert']=TRUE;
				
					$permissions['ProgUpdateDetail_Controller_update']=TRUE;
				
					$permissions['ProgUpdateDetail_Controller_delete']=TRUE;
				
					$permissions['ProgUpdateDetail_Controller_get_list']=TRUE;
				
					$permissions['ProgUpdateDetail_Controller_get_object']=TRUE;
				
					$permissions['ProgUpdateDetail_Controller_get_for_user']=TRUE;
				
					$permissions['QualityPassport_Controller_insert']=TRUE;
				
					$permissions['QualityPassport_Controller_update']=TRUE;
				
					$permissions['QualityPassport_Controller_delete']=TRUE;
				
					$permissions['QualityPassport_Controller_get_list']=TRUE;
				
					$permissions['QualityPassport_Controller_get_object']=TRUE;
				
					$permissions['QualityPassport_Controller_get_object_or_last']=TRUE;
				
					$permissions['QualityPassport_Controller_complete_vid_smesi_gost']=TRUE;
				
					$permissions['QualityPassport_Controller_complete_uklad']=TRUE;
				
					$permissions['QualityPassport_Controller_complete_sohran_udobouklad']=TRUE;
				
					$permissions['QualityPassport_Controller_complete_kf_prochnosti']=TRUE;
				
					$permissions['QualityPassport_Controller_complete_prochnost']=TRUE;
				
					$permissions['QualityPassport_Controller_complete_naim_dobavki']=TRUE;
				
					$permissions['QualityPassport_Controller_complete_aeff']=TRUE;
				
					$permissions['QualityPassport_Controller_complete_krupnost']=TRUE;
				
					$permissions['QualityPassport_Controller_complete_reg_nomer_dekl']=TRUE;
				
					$permissions['Client1c_Controller_complete']=TRUE;
				
					$permissions['Firm1c_Controller_get_list']=TRUE;
				
					$permissions['Firm1c_Controller_get_object']=TRUE;
				
					$permissions['ClientDebt_Controller_get_list']=TRUE;
				
					$permissions['ClientDebt_Controller_get_object']=TRUE;
				
					$permissions['ClientDebt_Controller_update_from_1c']=TRUE;
				
					$permissions['ShipmentMedia_Controller_get_list']=TRUE;
				
					$permissions['ShipmentMedia_Controller_get_object']=TRUE;
				
					$permissions['SrvStatistics_Controller_get_statistics']=TRUE;
				
					$permissions['ChatStatus_Controller_insert']=TRUE;
				
					$permissions['ChatStatus_Controller_update']=TRUE;
				
					$permissions['ChatStatus_Controller_delete']=TRUE;
				
					$permissions['ChatStatus_Controller_get_list']=TRUE;
				
					$permissions['ChatStatus_Controller_get_object']=TRUE;
				
					$permissions['UserChatStatus_Controller_set']=TRUE;
				
					$permissions['UserChatStatus_Controller_get_select_list']=TRUE;
				
					$permissions['UserChat_Controller_get_object']=TRUE;
				
					$permissions['UserChat_Controller_get_user_list']=TRUE;
				
					$permissions['UserChat_Controller_send']=TRUE;
				
					$permissions['UserChat_Controller_send_media']=TRUE;
				
					$permissions['UserChat_Controller_get_history']=TRUE;
				
					$permissions['UserChatLastOpen_Controller_update']=TRUE;
				
					$permissions['UserChatMessageView_Controller_set_all_viewed']=TRUE;
				
return array_key_exists($contrId.'_'.$methId,$permissions);
}
?>