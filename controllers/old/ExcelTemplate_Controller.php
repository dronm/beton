...
class ExcelTemplate_Controller extends ControllerSQL{
	...
	private static function replaceImageByName($sheet, $imageNameToReplace, $newImagePath) {
		// Get all drawings (images) in the sheet
		$drawings = $sheet->getDrawingCollection();
		
			file_put_contents(OUTPUT_PATH.'picts.txt',"***Looking for:".var_export($imageNameToReplace,true).PHP_EOL,FILE_APPEND);
		/* throw new Exception($imageNameToReplace); */
		$found = false;
		$cnt = 0;
		foreach ($drawings as $key => $drawing) {
			// Check if it's a regular Drawing and name matches
				/* throw new Exception("get a drawing by name:".var_export(get_class($drawing), true)); */
				/* throw new Exception("get a drawing by name:".var_export($drawing->getDescription(), true).",name:".var_export($drawing->getCoordinates(), true)); */
			file_put_contents(OUTPUT_PATH.'picts.txt','got:'.var_export($drawing->getCoordinates(),true).PHP_EOL,FILE_APPEND);
			if (
				/* ($drawing instanceof Drawing || $drawing instanceof MemoryDrawing) */
		...
		if(!$found){
			throw new Exception("image not found by name ".$imageNameToReplace." count:".$cnt);
		}
	}
