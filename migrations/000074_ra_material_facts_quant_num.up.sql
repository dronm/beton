...
alter table ra_material_facts ALTER COLUMN quant TYPE numeric;
alter table rg_material_facts ALTER COLUMN quant TYPE numeric;
...
CREATE OR REPLACE VIEW material_fact_balance_corrections_list AS
	SELECT
		...
		mat_f.quant * (CASE WHEN mat_f.deb THEN 1 ELSE -1 end) AS quant_dif
	...
	LEFT JOIN ra_material_facts mat_f ON 
		mat_f.doc_type = 'material_fact_balance_correction'::doc_types
		AND mat_f.doc_id = t.id
		AND mat_f.material_id = t.material_id
