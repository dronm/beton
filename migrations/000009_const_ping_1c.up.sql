BEGIN;
	...
	CREATE TABLE IF NOT EXISTS const_ping1c
	(name text, descr text, val json,
		val_type text,ctrl_class text,ctrl_options json, view_class text,view_options json);

	INSERT INTO const_ping1c (name,descr,val,val_type,ctrl_class,ctrl_options,view_class,view_options) VALUES (
		'1C ping'
		,'1C ping'
		,NULL
		,'JSON'
		...
		--constant get value
	CREATE OR REPLACE FUNCTION const_ping1c_val()
	RETURNS json AS
	$BODY$
		SELECT val::json AS val FROM const_ping1c LIMIT 1;
	$BODY$
	LANGUAGE sql STABLE COST 100;

	--constant set value
	CREATE OR REPLACE FUNCTION const_ping1c_set_val(JSON)
	RETURNS void AS
	$BODY$
		UPDATE const_ping1c SET val=$1;
	$BODY$
	LANGUAGE sql VOLATILE COST 100;

	--edit view: all keys and descr
	CREATE OR REPLACE VIEW const_ping1c_view AS
	SELECT
		'ping1c'::text AS id
		,t.name
		,t.descr
