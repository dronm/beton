	-- ********** constant value table  water *************
	BEGIN;

	CREATE TABLE const_water
	(name text, descr text, val json,
		val_type text,ctrl_class text,ctrl_options json, view_class text,view_options json);
	INSERT INTO const_water (name,descr,val,val_type,ctrl_class,ctrl_options,view_class,view_options) VALUES (
		'Вода'
		,'Water'
		,'{"keys" : {"id" : "12"}, "descr" : "Вода", "dataType" : "concrete_types"}'
		,'JSON'
		,NULL
		,NULL
		,NULL
		...
		--constant get value
	CREATE OR REPLACE FUNCTION const_water_val()
	RETURNS json AS
	$BODY$
		SELECT val::json AS val FROM const_water LIMIT 1;
	$BODY$
	LANGUAGE sql STABLE COST 100;
	--constant set value
	CREATE OR REPLACE FUNCTION const_water_set_val(JSON)
	RETURNS void AS
	$BODY$
		UPDATE const_water SET val=$1;
	$BODY$
	LANGUAGE sql VOLATILE COST 100;
	--edit view: all keys and descr
	CREATE OR REPLACE VIEW const_water_view AS
	SELECT
		'water'::text AS id
		,t.name
		,t.descr
