		...
		if($vehicleState=='busy' || $vehicleState=='left_for_dest'){
			...
			$cashe_ar = $dbLink->query_first(sprintf(
				...
				zone_data AS (
					...
					WHERE dest.id = (SELECT dest_id FROM dest_q)
				)			
				,veh_last_points AS (
					SELECT
						json_agg(
				...
				,client_zone AS (
					...
					WHERE cl_dest.client_id = (
						...
						AND cl_dest.destination_id = (SELECT dest_id FROM dest_q)
				)
				SELECT
			...
			//берем ближйшую площаду
			$cashe_ar = $dbLink->query_first(sprintf(
				"WITH
				last_pos AS (SELECT lon,lat from car_tracking
							 WHERE car_id = '%s' AND gps_valid = 1
				...
				,zone_data AS (SELECT
					...
					ORDER BY 
						ST_Distance(
							st_geomFromText('POINT('|| (SELECT lon FROM last_pos) ||' '|| (SELECT lat FROM last_pos) ||')', 0)
							,d.zone
						)
					LIMIT 1)
				,veh_last_points AS (
