...
MaterialStoreForOrderList_View.prototype.setData = function(modelStores){
	...
	if(!is_init){
		...
		while(modelStores.getNextRow()){
			if (!modelStores.getFieldValue("material_id")) {
				continue;
			}
...
MaterialStoreForOrderList_View.prototype.drawStore = function(storeNode,storeContNode,posLeft,posTop,fillPercent){
	...
	//filling
	var fill_w = Math.floor( store_width * fillPercent / 100) - fill_tolerance*2;
	cx.fillStyle = "#8c8c8c";//"#E5E5E5";//"grey";
	cx.lineWidth = "0.5";
	cx.beginPath();
	cx.moveTo(posLeft + fill_tolerance, posTop + fill_tolerance);
	cx.lineTo(fill_w, posTop + fill_tolerance);
	cx.lineTo(fill_w, posTop + (store_height - fill_tolerance*2) );
	cx.lineTo(posLeft + fill_tolerance, (store_height - fill_tolerance*2));
	//posTop + fill_tolerance + store_height - fill_tolerance
	...
	//( (store_height)/2+100)
	storeContNode.style = "position:relative;top:-20px;left:5px";
}
