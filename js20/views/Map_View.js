...
Map_View.prototype.toDOM = function(parent){
	...
	if(this.m_paramVehicle && (this.m_paramCurrentPosition_Model||this.m_paramTrack_Model||this.m_paramZone_Model||this.m_paramRoute_Model)){
		...
		if(this.m_paramRoute_Model && this.m_paramRoute_Model.getNextRow()){
			this.addGuessedRoute(this.m_paramRoute_Model.getFieldValue("route_rest"), "Построен маршрут");			
		}
			...
			this.subscribeToSrvEvents({
				...
				,"onEvent":function(json){
					...
					if(json.eventId && json.eventId == "Vehicle.route_redraw."+self.m_curTrackerId){
						...
						if(json.params.long_route_rest===true){
							self.fetchGuessedRoute("Маршрут перестроен");
							
						}else if(json.params.route_rest){
...
Map_View.prototype.onGetPosData = function(resp,callBack){
	...
	if(m && m.getNextRow()){
		this.addGuessedRoute(m.getFieldValue("route_rest"), "Построен маршрут");	
	}
...
/**
 * routGeom = encoded line
 * ZnUCgGl1AoAAAAAARwAAAAAAAAAAAAAAAAAAACEL70EAAAAAAAAAAAAAAABHAAAAAAAAAAAAAAALAAAAyWroA1tcaAPJaugDW1xoAwAALwW6CmHl
 * Более не используется всегда приходит текст! array(lon lat)
 */
/*Map_View.prototype.addGuessedRouteOnEncodedPolyline = function(routGeom){
	var points = decodeLine(routGeom.replace(/\\\\/g, "\\"));//ext function				
	this.addGuessedRouteOnPoints(points);
}
*/
/**
 * Text array(lon lat)
...
Map_View.prototype.addGuessedRoute = function(routeText, noteMsg){
	...
	if(noteMsg){
		window.showTempNote(noteMsg,null,5000);	
	}
}

Map_View.prototype.fetchGuessedRoute = function(msg){
	var self = this;
	(window.getApp().getServConnector()).execRequest(
		true,
		null,
		true,
		function(error_n,error_s,resp){
			var m = resp.getModel("RouteRest_Model");
			if(m && m.getNextRow()){
				self.addGuessedRoute(m.getFieldValue("route_rest"), msg);
			}
		},
		"application/json",null,
		"route/long_route_rest.php?"+this.m_curTrackerId
	);
