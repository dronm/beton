...
//console.log("AppBeton.prototype.sendNotification Sending notification to contact ", ref)
//alert("Sending notification to contact");
//return;
	var view = new Notification_View("notification:cont",{
		"ref":ref
	});
	...
	if(this.showTmChat()){
		...
		this.m_appSrv.subscribe({
			...
			,"onEvent":function(json){				
				//out message, on error
				/*if(json.eventId == "TmOutMessage.sent"
				...
				console.log("AppBeton.prototype.subscribeToEventSrv PLAY!!!")
				*/
				self.m_chat.onNewMessage(json);
				
				/*
				var sender = "";
				if(json.params.ext_obj && json.params.ext_obj.getDescr){
					sender = json.params.ext_obj.getDescr();
				}else if(json.params.sender){
					sender = json.params.sender;
				}
				window.showNote({
					"elements":[
						new Control(CommonHelper.uniqid(),"DIV",{
							"value":(sender? sender+": ":"") +json.params.text,
							"attrs":{"style":"cursor:pointer;","title":"Кликом мышки можно отправить ответ"},
							"events":{
								"click":(function(extObj){
									return function(){
										self.sendNotification(extObj);
									}
								})(json.params.ext_obj)
							}
						})
