 ...
 */
function Item1cEdit(id,options){

	options = options || {};	
	if (options.labelCaption!=""){
		options.labelCaption = options.labelCaption || "Номенклатура 1с:";
	}
	if (options.title!=""){
		options.title = options.title || "Ссылка на номенклатару 1с";
	}
	if (options.placeholder!=""){
		options.placeholder = options.placeholder || "Наименование номенклатары в 1с";
	}
	...
	options.acPublicMethod = options.acController.getPublicMethod("complete_item");
	options.acModel = new ModelXML("Item1cList_Model", {
		"fields":["name", "ref"]
	});
	...
	Item1cEdit.superclass.constructor.call(this,id,options);
}
extend(Item1cEdit,EditRef);
