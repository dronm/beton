...
function VehicleScheduleEdit_View(id,options){
	...
	options.addElement = function(){
		...
		this.addElement(new ButtonCmd(id+":cmdApplyProdBase",{
			"caption":"Установить базу",
			"title":"Установить выбранную базу для выбраных транспортных средств в рабочие смены.",
			"onClick":function(){
				self.cmdApplyProdBase();
			}
		}));	
...
VehicleScheduleEdit_View.prototype.cmdApplyTemplateCont = function(nodes){
	...
	for(var i = 0; i < nodes.length; i ++ ){
		...
		var pbase_nodes = DOMHelper.getElementsByAttr("owner_schedule_prod_base", nodes[i].node, "class", true);
		if(pbase_nodes && pbase_nodes.length){
			DOMHelper.delNode(pbase_nodes[0]);
		}
	
		if(nodes[i].shift){
			DOMHelper.addClass(nodes[i].node, "owner_schedule_shift");
			this.addDriverNode(nodes[i].node, drv_ref);
			this.addProdBaseNode(nodes[i].node, pbase_ref);			
		}else{
			if(nodes[i].weekend){
		...
		if(this.checked){
			...
			for(var i = 0; i < nodes.length; i ++ ){
				...
				if(is_shift && !prod_base_id){
					//prod_base_id MUST be
					var prod_base_ref = self.getElement("tmpl_prod_base").getValue();
					if(!prod_base_ref || prod_base_ref.isNull()){
						self.getElement("tmpl_prod_base").setNotValid("Не выбран");
...
VehicleScheduleEdit_View.prototype.cmdApplyProdBaseCont = function(dNodes, prodBaseRef){
	for(i = 0; i < dNodes.length; i++){
		dNodes[i].setAttribute("prod_base_id", prodBaseRef.getKey());
		dNodes[i].setAttribute("title", prodBaseRef.getDescr());
		dNodes[i].textContent = prodBaseRef.getDescr().substring(0, this.BASE_SHORT_CHAR_COUNT);					
	}
}

VehicleScheduleEdit_View.prototype.cmdApplyProdBase = function(){
	...
	var prod_base_nodes = [];//prod base nodes
	var vehicles = [];//array of objects {"vehicle_id":VH_ID, "days":[{"date":DATE, "shift":true, "prod_base_id":PROD_BASE_ID}]}	
	$(".owner_schedule_vehicle_mark").each(function(index) {
		if(this.checked){				
			var add_vh = false;
			var days = [];
			...
			for(var i = 0; i < nodes.length; i ++ ){
				if($(nodes[i]).hasClass("owner_schedule_shift")){
					var drv_nodes = DOMHelper.getElementsByAttr("owner_schedule_prod_base", nodes[i], "class", true);
					if(drv_nodes && drv_nodes.length){
						prod_base_nodes.push(drv_nodes[0]);
						days.push({
							"date": nodes[i].getAttribute("dt"),
							"shift": true,
	...
	var self = this;
	this.setScheduleServerCall(vehicles, function(){
		self.cmdApplyProdBaseCont(prod_base_nodes, prod_base_ref);
	});	
}
...
VehicleScheduleEdit_View.prototype.changeProdBase = function(forDate, callback){
	(new WindowFormModalBS(this.getId()+":changeProdBase", {
		"contentHead": "База на " + DateHelper.format(forDate, "d/m/Y"),
		"content":new View(this.getId()+":changeProdBase:v", {
			"elements":[
				new ProductionBaseEdit(this.getId()+":changeProdBase:v:prod_base",{
					"required":true,
					"focus":true
				})	
			]
...
VehicleScheduleEdit_View.prototype.markDayOnNode = function(n){
	...
	if(!is_busy){
		...
		//prod base
		var prod_base_n = $(n).find(".owner_schedule_prod_base").eq(0);
		var prod_base_id = prod_base_n.attr("prod_base_id");
		var prod_base_ref;
		if(!prod_base_id){
			//tempate prod base
			...
			prod_base_ref = new RefType({"key":{"id": prod_base_id}, "descr": prod_base_n.attr("title")});
		}
		...
		if(driver_id && prod_base_id){
		...
		}else if(!prod_base_id && driver_id){
			//select prod base
			var self = this;
			...
			//select driver && select prod base
			var self = this;
			this.changeDriver(schedule_date, function(drvRef){				
...
VehicleScheduleEdit_View.prototype.changeProdBaseClick = function(prodBaseNode){
	var self = this;
	var n = prodBaseNode.parentNode;
	var schedule_date = DateHelper.strtotime(n.getAttribute("dt"));			
	this.changeProdBase(schedule_date, function(prodBaseRef){
		if($(n.parentNode).find(".owner_schedule_vehicle_mark").first().get()[0].checked){
			//multy vehicle mode
			...
			$(".owner_schedule_vehicle_mark").each(function(index) {
				if(this.checked){				
					...
					var drv_nodes = DOMHelper.getElementsByAttr("owner_schedule_prod_base", nodes[0], "class", true);
					if(!drv_nodes || !drv_nodes.length){
						//Может быть при групповом добавлении, если у кого-то нет базы
						drv_nodes = [self.addDriverNode(prod_base_nodes[0], prodBaseRef)];
					}					
					prod_base_nodes.push(drv_nodes[0]);
					vehicles.push(nodes[0].parentNode.getAttribute("vehicle_id"));
				}
			...
			self.setSchedule(
				...
				is_busy,
				function(){
					for(i = 0; i < prod_base_nodes.length; i++){
						prod_base_nodes[i].setAttribute("prod_base_id", prodBaseRef.getKey());
						prod_base_nodes[i].setAttribute("title", prodBaseRef.getDescr());
						prod_base_nodes[i].textContent = prodBaseRef.getDescr().substring(0, self.BASE_SHORT_CHAR_COUNT);					
					}
				}
			);
			...
			self.setSchedule(
				...
				true,
				function(){
					prodBaseNode.setAttribute("prod_base_id", prodBaseRef.getKey());
					prodBaseNode.setAttribute("title", prodBaseRef.getDescr());
					prodBaseNode.textContent = prodBaseRef.getDescr().substring(0, self.BASE_SHORT_CHAR_COUNT);
				}
			);				
		}
	});	
			...
			this.setSchedule(
				...
				function(){
					for(i = 0; i < d_nodes.length; i++){
						...
						$(d_nodes[i]).find(".owner_schedule_prod_base").eq(0).remove();						
					}
				}
			);
	...
	*/
	...
	var m = new ModelXML("VehicleOwnerSchedule_Model", {
		"fields":[
			...
			new FieldJSON("prodction_bases_ref")
		],
	...
	while(m.getNextRow()){
		...
		var bs = m.getFieldValue("prodction_bases_ref").getDescr();
		if(old_vh != vh){
			//new vehicle = new row
		...
		row.DAYS.push({
			...
			"PROD_BASE": bs,
			"PROD_BASE_SHORT": bs.substring(0, this.BASE_SHORT_CHAR_COUNT),
			"PROD_BASE_ID": m.getFieldValue("prodction_bases_ref").getKey("id"),
		});
