...
function PrintPutevoiListBtn(id,options){
	...
	options.title="Напечатать путевой лист";
	...
	PrintPutevoiListBtn.superclass.constructor.call(this,id,options);
}
//ViewObjectAjx,ViewAjxList
extend(PrintPutevoiListBtn,Button);
...
/* protected*/
PrintPutevoiListBtn.prototype.m_grid;

/* public methods */
PrintPutevoiListBtn.prototype.onClick = function(){
	var id,pm;
	if(this.m_cmd){
		this.m_grid.setModelToCurrentRow();
		id = this.m_grid.getModel().getFieldValue("id");
		pm = this.m_grid.getReadPublicMethod().getController().getPublicMethod("shipment_putevoi_list");
	}
	else{
		...
		id = keys.id;
		pm = this.gridColumn.getGrid().getReadPublicMethod().getController().getPublicMethod("shipment_putevoi_list");
	}
