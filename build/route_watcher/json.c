    ...
    while(*str != '\0') {
        ...
        if(*str == '{') {
            ...
            if(*str == '{') {
                ...
                tempPtr.value = new(JSONValue);                
                string tmp_str = malloc( (i+1) * sizeof(character));
                if(!tmp_str){
	            freeJSONFromMemory(obj);
	            return NULL;
                }
                memcpy(tmp_str, str, i * sizeof(character));
                tmp_str[i] = '\0';
                
                int dig_sep = strNextOccurence(tmp_str, '.');
                if(dig_sep == -1){
                	tempPtr.type = JSON_INT;
                	tempPtr.value->intValue = atoi(tmp_str);
                }
                else{
                	tempPtr.type = JSON_FLOAT;
                	tempPtr.value->floatValue = atof(tmp_str);
                }               
                free(tmp_str);
