...
BEGIN
	...
	schedule_work AS (
		SELECT
			...
			bs.vehicle_id,
			bs.driver_id,
			min(r.st_assigned) AS work_start,
...
BEGIN
	...
	daily_work AS (
		SELECT
			bs.schedule_date AS day,
			bs.vehicle_id,
			bs.driver_id,
			sum(
				CASE
					WHEN r.run_time IS NULL THEN 0
					ELSE extract(
						epoch FROM (
							r.run_time - current_date::timestamp without time zone
						)
					)
				END
			) AS seconds_worked
		...
		SELECT
			...
			json_agg(
				json_build_object(
					...
					'is_day_off', extract(isodow FROM d.day) IN (6, 7),
					'hours', 
					---coalesce(floor(dw.seconds_worked / 3600), 0)::int
					CASE
						WHEN dw.seconds_worked IS NULL THEN 0
						ELSE floor((dw.seconds_worked + 1800) / 3600)::int
					END
				)
				ORDER BY d.day
			) AS time_data
