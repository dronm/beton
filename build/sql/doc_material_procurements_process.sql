...
BEGIN
	IF (TG_WHEN='BEFORE' AND TG_OP='INSERT') THEN
		...
		--for weigh app leave as is, for others default process_date_time = date_time
		IF NEW.process_date_time IS NULL AND NEW.weigh_app=FALSE THEN
			NEW.process_date_time = NEW.date_time;
		END IF;	
	...
	ELSIF (TG_WHEN='AFTER') AND (TG_OP='INSERT' OR TG_OP='UPDATE') THEN					
		IF NEW.process_date_time IS NOT NULL AND NEW.process_date_time > '2024-01-01T00:00:00' THEN
			IF (TG_OP='INSERT') THEN						
				--log
				PERFORM doc_log_insert('material_procurement'::doc_types,NEW.id,NEW.process_date_time);
			END IF;

			--register actions ra_materials
			reg_act.date_time			= NEW.process_date_time;
			reg_act.deb					= true;
			...
			reg_material_facts.date_time			= NEW.process_date_time;
			...
			AND NEW.cement_silos_id IS NOT NULL THEN
				...
				reg_cement.date_time			= NEW.process_date_time;
				reg_cement.deb					= true;
	...
	ELSIF (TG_WHEN='BEFORE' AND TG_OP='UPDATE') THEN
		...
		IF NEW.process_date_time <> OLD.process_date_time THEN
			PERFORM doc_log_update('material_procurement'::doc_types,NEW.id,NEW.process_date_time);
		END IF;
